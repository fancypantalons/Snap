...
use SnapLib::GtkTextPrinter;
use SnapLib::GtkInput;
...
sub setup_gui
{
  ...
  push @{ $code_hash{&MSG_DL_START} }, (sub { &detach_download(@_, $list); }, sub { gtk_add_dl(@_, $list) });
  ...
  my $vbox = Gtk::VBox->new(0, 0);
  my $hbox = Gtk::HBox->new(0, 0);
  my $button = Gtk::Button->new("Cancel");
  my $align = Gtk::Alignment->new(0.5, 0.5, 1, 1);
  ...
  push @{ $code_hash{&MSG_UL_START} }, (sub { detach_upload(@_, $list) }, sub { gtk_add_ul(@_, $list) });
  ...
  tie *STDOUT, 'SnapLib::GtkTextPrinter', $textbox;
  ...
  push @{ $code_hash{&MSG_SERVER_CONN} }, \&server_conn_add;
  push @{ $code_hash{&MSG_SERVER_CONN_ERR} }, \&server_conn_err;

  $code_hash{&MSG_SERVER_STATS} = [ \&update_stats ];
  $code_hash{&MSG_DISCONNECT} = [ \&gtk_server_disconnect ];

  push @{ $command_hash{"/reconnect"} }, \&gtk_server_reconnect;

  $napster_tag = Gtk::Gdk->input_add(fileno($napster_sock), 'read', 
                                     sub { check_napster($napster_sock); });

  Gtk::Gdk->input_add(fileno($server_sock), 'read', 
                      sub { setup_server_conn($server_sock); });

  # Gtk->idle_add(\&call_polled_funcs);

  main Gtk;
}

sub call_polled_funcs
{
  my $func;
...
{
  Gtk::Gdk->input_remove($napster_tag);
...
sub gtk_server_reconnect
{
  Gtk::Gdk->input_remove($napster_tag) if (defined $napster_tag);
...
sub attach_download
{
  ...
  if (defined $$dl{"tag"})
    {
      Gtk::Gdk->input_remove($$dl{"tag"});
    }

  $$dl{"tag"} = Gtk::Gdk->input_add(fileno($$dl{"socket"}), 'read', 
				    sub { download_handshake($napster_sock, $$dl{"socket"}) });
...
sub detach_download
{
  my ($sock, $dl, $new_dl, $list) = @_;
  my $rownum = $list->find_row_from_data($dl);

  Gtk::Gdk->input_remove($$dl{"tag"});

  $$new_dl{"tag"} = Gtk::Gdk->input_add(fileno($$new_dl{"read"}), 'read', 
					sub { transfer_cmd($napster_sock, $$new_dl{"read"}); });

  $list->set_row_data($rownum, $new_dl);
}
...
sub gtk_add_dl
{
  ...
  $list->set_text($rownum, 1, $$new_dl{"size"});
  $list->set_text($rownum, 2, $$new_dl{"received"});
...
sub gtk_rem_dl
{
  ...
  Gtk::Gdk->input_remove($$dl{"tag"}) if (defined $$dl{"tag"});
}
...
sub attach_upload
{
  ...
  if (defined $$ul{"tag"})
    {
      Gtk::Gdk->input_remove($$ul{"tag"});
    }

  $$ul{"tag"} = Gtk::Gdk->input_add(fileno($$ul{"socket"}), 'read', 
				    sub { upload_handshake($napster_sock, $$ul{"socket"}) });

  $rownum = $list->append($$ul{"filename"}, $$ul{"size"}, 
			  $$ul{"received"}, "Initializing", "");
...
sub detach_upload
{
  my ($sock, $ul, $new_ul, $list) = @_;

  Gtk::Gdk->input_remove($$ul{"tag"});

  $$new_ul{"tag"} = Gtk::Gdk->input_add(fileno($$new_ul{"read"}), 'read', 
					sub { transfer_cmd($napster_sock, $$new_ul{"read"}); });
}
...
sub gtk_add_ul
{
  ...
  $list->set_text($rownum, 1, $$new_ul{"size"});
  $list->set_text($rownum, 2, $$new_ul{"sent"});
  $list->set_text($rownum, 3, "Transferring");

  $list->set_row_data($rownum, $new_ul);
}
...
sub server_conn_add
{
  ...
  $$conn{"tag"} = Gtk::Gdk->input_add(fileno($$conn{"socket"}), 'read', 
				      sub { handle_server_request($$conn{"socket"}) });

  debug_print("UL", "Attaching $$conn{tag}...\n");
}
...
sub server_conn_err
{
  ...
  Gtk::Gdk->input_remove($$conn{"tag"});

  debug_print("UL", "Detaching $$conn{tag}...\n");
}
...
sub label_clicked
{
  ...
  $printer = SnapLib::GtkTextPrinter->new($textbox);
